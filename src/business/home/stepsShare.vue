<template>
    <div class="share-steps" id="shareNew">
        <div class="top">
            <img :src="imgPath" alt="">
        </div>
        <div class="container">
            ...
            <div class="news-item">
                <div><label>总步数:</label>{{steps}} </div>
                <div><label>总公里:</label>{{distance}}</div>
                <div><label>我的糖果:</label>{{myCoin}}</div>
            </div>
        </div>
        ...
        <div class="shareBtn" @click="share('weixin')" >分享</div>
        ...
        methods: {
            shareLoad(){
                ...
                users.shareAction(this,function(json,memberid){
                    ...
                    _this.distance = _this.db.loadTodayDistance(_this,memberid);
            ...
            share(){
                users.shareImg(this,0,$('header').outerHeight(true),api.winWidth,$('#shareNew').height())
            }
        },
        mounted() {
            $(".code").width(87)
